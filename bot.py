...
async def play(ctx, *,URL:str=None):
    ...
    if not vc.is_playing() and not vc.is_paused():
        ...
        embed=discord.Embed(title=f'Добавлено в очередь: {title}',description=URL, colour=0xDE6D07)
...
async def PlayMusic(ctx):
    while len(queue)>0:
        song=queue[0]
        vc.play(discord.FFmpegPCMAudio(source=song.SourceUrl, **FFMPEG_OPTIONS))
        embed=discord.Embed(title=f'Сейчас играет: {song.title}',description=song.url, colour=0x00FF00)
...
async def stop(ctx):
    if voice_channel.id!=ctx.message.author.voice.channel.id:
        ...
        if len(queue)>0:
            song=queue[0]
            embed=discord.Embed(title='Песня скипнута:',description=song.title,colour=0xFF0000)
            vc.stop()
            await ctx.send(embed=embed)
        elif len(queue)==0:
            await ctx.send('Музыка сейчас не играет')
...
async def song(ctx):
    if len(queue)==0:
        embed=discord.Embed(title='В данный момент музыка не воспроизводится', colour=0xFF0000)
        await ctx.send(embed=embed)
    else:
        song=queue[0]
        embed=discord.Embed(title='Текущая песня:',description=f'{song.title}\n{song.url}', colour=0x00FF00)
...
async def _queue(ctx):
    queue_=""
    if len(queue)==0:
        embed=discord.Embed(title='Очередь пуста', colour=0xFF0000)
        await ctx.send(embed=embed)
    else:
        for i in range(len(queue)):
            song=queue[i]
            queue_=queue_+song.title+'          '+song.duration+'\n'
        embed = discord.Embed(title='Текущая очередь:',description=queue_, colour=0x00FF00)
        await ctx.send(embed=embed)
...
async def gif(ctx,tag=None):
    if tag==None:
        response = requests.get("https://g.tenor.com/v1/search?limit=50&key=O4UDCVI14VWL")
    else:
...
async def anime(ctx,tag=None):
    if tag==None:
        response = requests.get("https://safebooru.org/index.php?page=dapi&s=post&q=index&limit=100")
    else:
        payload = {'tags': tag}
        response = requests.get("https://safebooru.org/index.php?page=dapi&s=post&q=index&limit=100", params=payload)
